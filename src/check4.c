...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/07 22:08:09 by ybesbes           #+#    #+#             */
/*   Updated: 2021/01/07 22:08:16 by ybesbes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_values_if_plkey(t_config_map *element, t_vars *vars)
{
	if (ft_strnstr(element->key, "pl", 2))
	{
		...
		{
			vars->error_msg =
				ft_strdup("number of values for pl key is not correct");
			return (-1);
		}
...
int	check_values_if_sq_key(t_config_map *element, t_vars *vars)
{
	if (ft_strnstr(element->key, "sq", 2))
	{
		...
		{
			vars->error_msg =
				ft_strdup("number of values for sq key is not correct");
			return (-1);
		}
		if (check_if_value_is_tab_of__floats_or_int(element->value[0]) != 1 ||
			check_n_limitted_floats_or_int(element->value[1], -1, 1, 3) != 1 ||
			(check_if_float(element->value[2]) != 1 &&
			value_is_int(element->value[2]) != 1) ||
...
int	check_values_if_cy_key(t_config_map *element, t_vars *vars)
{
	if (ft_strnstr(element->key, "cy", 2))
	{
		...
		{
			vars->error_msg =
				ft_strdup("number of values for cy key is not correct");
			return (-1);
		}
		if (check_if_value_is_tab_of__floats_or_int(element->value[0]) != 1 ||
			check_n_limitted_floats_or_int(element->value[1], -1, 1, 3) != 1 ||
			(check_if_float(element->value[2]) != 1 &&
			value_is_int(element->value[2]) != 1) ||
			(check_if_float(element->value[3]) != 1 &&
			value_is_int(element->value[3]) != 1) ||
...
int	check_values_if_trkey(t_config_map *element, t_vars *vars)
{
	if (ft_strnstr(element->key, "tr", 2))
	{
		...
		{
			vars->error_msg =
				ft_strdup("number of values for tr key is not correct");
			return (-1);
		}
		if (check_if_value_is_tab_of__floats_or_int(element->value[0]) != 1 ||
