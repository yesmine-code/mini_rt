...
t_color get_intersection_color(const t_obj_info *obj_info, t_scene *scene)
{
	...
	{
	case SPHERE:
		inter_color = ((t_sphere *)get_from_list_by_id(scene->spheres, obj_info->object_id))->color;
		break;
	case PLANE:
		inter_color = ((t_plane *)get_from_list_by_id(scene->planes, obj_info->object_id))->color;
		break;
	case TRIANGLE:
		inter_color = ((t_triangle *)get_from_list_by_id(scene->triangles, obj_info->object_id))->color;
		break;
	case SQUARE:
		inter_color = ((t_square *)get_from_list_by_id(scene->squares, obj_info->object_id))->color;
		break;
	case CYLINDRE:
		inter_color = ((t_cylindre *)get_from_list_by_id(scene->cylindres, obj_info->object_id))->color;
		break;
	default:
		break;
	}
