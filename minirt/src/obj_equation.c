...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/07 22:18:56 by ybesbes           #+#    #+#             */
/*   Updated: 2021/01/07 22:18:57 by ybesbes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_inter_sphere(t_sphere *s, t_ray *r, double *t)
{
	...
	eq.c = ft_get_norm_carre(ft_vec_sub(r->origine, s->origine))
			- (s->rayon * s->rayon);
...
int	ft_solve_triangle_eq(t_vector u, t_vector v, t_vector w)
{
	...
	detm = ft_get_norm_carre(u) * ft_get_norm_carre(v)
			- ft_vec_dot(u, v) * ft_vec_dot(u, v);
	detb = ft_vec_dot(w, u) * ft_get_norm_carre(v)
			- ft_vec_dot(w, v) * ft_vec_dot(u, v);
	eq.b = detb / detm;
	detg = ft_get_norm_carre(u) * ft_vec_dot(w, v)
			- ft_vec_dot(u, v) * ft_vec_dot(w, u);
	eq.c = detg / detm;
	eq.a = 1 - eq.b - eq.c;
...
int	inter_triangle(t_triangle *tr, t_ray *r, double *t)
{
	...
	*t = ft_vec_dot(ft_vec_sub(tr->p3, r->origine),
		ft_vec_norm(tr->direction)) /
...
int	inter_square(t_square *sq, t_ray *r, double *t)
{
	...
	if (!(eq.a == 0 || (eq.b < 0 && eq.a < 0) || (eq.b > 0 && eq.a > 0)))
	{
		...
		{
			eq.t2 = sq->hauteur / 2;
			d = ft_vec_sub(ft_vec_add(ft_vec_mult_d(eq.t1, r->direction),
				r->origine), sq->origine);
